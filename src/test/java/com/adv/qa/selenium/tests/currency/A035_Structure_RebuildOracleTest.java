...
/**
 * @author              :   Chetna
 * Test Reference No	: 	A035 Structure Rebuild (EP3) for Oracle
 * Purpose              :   Run Structure Rebuild
 * Date					:   08-03-2017
 * ACCESS               :   Submit EP3
 */

public class A035_Structure_RebuildOracleTest extends BaseTest{
	/*Launch the browser*/
	@BeforeClass
...
//		/*Create structure build details*/
		...
		String statBefore = currencyPage.getProcessDetailsOracle(process,structureList.get(1));
		
		Assert.assertEquals(testcases,statBefore, "2","Process has","entered task list");
		
		if(statBefore.equals("2")){;
			currencyPage.updateProcessOracle(process,structureList.get(1));
		}
		
		String statAfter = currencyPage.getProcessDetailsOracle(process,structureList.get(1));
		
		if(statAfter == null || statAfter.equals("3"))
		{
			value = true;			
		}
		Assert.assertTrue(testcases,value,"Process" +process,"performed on "+structureList.get(1));
